...
function makeCatalog(overrides?: TestCatalogOverrides): ComponentCatalog {
  const components: Record<string, ComponentEntry> = overrides?.components ?? {
    'r-table': makeEntry({
      ...
      description: '表格容器',
      props: [
        { name: 'dataViewKey', type: 'string', required: false, description: '表格 DataView 定位键' },
      ...
      binding: {
        ...
        examples: [{ type: 'r-table', props: { dataViewKey: 'orders@default' } }],
      },
      rootFields: [
        {
          name: 'currentRow',
...
describe('catalog-projections', () => {
  ...
  it('projectComponentSpec returns component spec for pageDesign/knowledge/guidePayload', () => {
    ...
    expect(spec!.props.some((prop) => prop.name === 'dataViewKey')).toBe(true)
    ...
    const catalog = makeCatalog({
      components: {
        'r-table': makeEntry({
          ...
          category: 'container',
          description: '表格容器',
          props: [{ name: 'dataViewKey', type: 'string', required: false }],
        }),
