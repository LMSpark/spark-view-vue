...
import type { ProjectState } from '@/views/dev-system/composables/types'
...
describe('canAdvance', () => {
  it('requirements → functions: fails if no analyzed requirement', () => {
  ...
  it('functions → navigation: fails if no module with pages', () => {
    const state = makeState({
  ...
  it('navigation → page-design: fails if navDirty', () => {
  ...
  it('page-design → verification: fails if no accepted proposal', () => {
...
describe('canRegress', () => {
  it('regressing to requirements with downstream data warns', () => {
    ...
    expect(result.allowed).toBe(false)
    expect('reason' in result && result.reason).toContain('失效')
  })
...
describe('canJumpTo', () => {
  ...
  it('skip forward: forbidden', () => {
    const state = makeState()
    const result = canJumpTo('requirements', 'navigation', state)
    expect(result.allowed).toBe(false)
    expect('reason' in result && result.reason).toContain('逐步')
  })
