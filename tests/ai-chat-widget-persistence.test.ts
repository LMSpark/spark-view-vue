...
describe('AiChatWidget persistence', () => {
  ...
  it('renders SSE diagnostics and opens FC call details', async () => {
    ...
    const wrapper = mount(AiChatWidget, {
      props: {
        ...
        externalToolLogs: [
          ...
          { type: 'info', tag: 'LLM → session.describe', text: '读取会话状态', timestamp: '2026-04-27T13:03:36.500Z' },
    ...
    expect(structured.toolLogs).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ tag: 'LLM → session.describe', title: 'LLM → session.describe', text: '读取会话状态' }),
    ]))
    ...
    expect(structured.fcCalls[0]).toMatchObject({ toolName: 'catalog.query', status: 'success' })
  ...
  it('batches SSE cache writes and persists the final structured snapshot', async () => {
    ...
    expect(snapshot.fcCalls?.[0]?.toolName).toBe('catalog.query')
