...
- **DataViewKey**：容器用 DataViewKey 定位 DataView，例如 `Users@default`；展示和动作读取 DataView 输出时使用 `dataViewKey + dataMember + dataField`，例如 `dataViewKey: "Orders@detail", dataMember: "currentRow", dataField: "total"`。
