...
async function main() {
  const { url } = parseArgs()
  const metadataPath = resolve(projectRoot, 'packages', 'spark-page-config', 'src', 'ai', 'payloads', 'component-catalog.json')

  // 检查文件存在
  if (!existsSync(metadataPath)) {
    console.error('❌ 未找到 packages/spark-page-config/src/ai/payloads/component-catalog.json')
