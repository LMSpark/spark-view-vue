...
- 编译期 `.d.ts` 仅在内存存在（虚拟前缀 `class-model-emit/`）；落盘 manifest 键与 shard 使用源码 repo 相对路径（`packages/.../foo.ts` → `files/packages/.../foo.ts.json`）；产物目录为 `generated/dts-class-model/**/*.json`。
