...
function Resolve-BailianToken {
  ...
  if (Test-Path -LiteralPath $settingsPath) {
    $settings = Get-Content -LiteralPath $settingsPath -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable -ErrorAction SilentlyContinue
...
function Restore-ClaudeEnvironment {
  $token = Resolve-BailianToken

  if ([string]::IsNullOrWhiteSpace($token)) {
    Write-Warning "Bailian API key was not found. Set ANTHROPIC_AUTH_TOKEN / ALIYUN_CODING_PLAN_API_KEY / DASHSCOPE_API_KEY in user environment first."
    return
  }
  ...
  if (Test-Path -LiteralPath $settingsPath) {
    ...
    $settingsEnv = $settings["env"]
    $settingsEnv.Remove("ANTHROPIC_API_KEY")
    $settingsEnv["ANTHROPIC_AUTH_TOKEN"] = $token
    $settingsEnv["ANTHROPIC_BASE_URL"] = $BailianBaseUrl
    $settingsEnv["ANTHROPIC_MODEL"] = $BailianModel
    $settingsEnv["ANTHROPIC_DEFAULT_HAIKU_MODEL"] = $BailianModel
    $settingsEnv["ANTHROPIC_DEFAULT_SONNET_MODEL"] = $BailianModel
    $settingsEnv["ANTHROPIC_DEFAULT_OPUS_MODEL"] = $BailianModel
    $settingsEnv["CLAUDE_CODE_SUBAGENT_MODEL"] = $BailianModel
    $settings | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $settingsPath -Encoding UTF8
  }

  $rootConfigPath = Join-Path $HOME ".claude.json"
  if (Test-Path -LiteralPath $rootConfigPath) {
    $rootConfig = Get-Content -LiteralPath $rootConfigPath -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable
    $rootConfig["hasCompletedOnboarding"] = $true
    $rootConfig | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $rootConfigPath -Encoding UTF8
  }
