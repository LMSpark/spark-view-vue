...
rule.json 允许将 dataKey / field / id / on / visible / disabled 等写在根级（便于阅读），绑定阶段（`buildPageChildren`）会**全部收入 `props`**，组件代码只需关心 `props`。
...
rule.json
  { type: "r-table", dataKey: "Users@rows", children: [...] }
    ↓ buildPageChildren()         ← 根级 dataKey/field 收入 props
...
`buildPageChildren` 以结构键黑名单（`type/props/children`）实现规范化：所有非结构根级字段（包括 `id/on/visible/disabled/dataKey/field` 等）一律收入 `props`。因此：
