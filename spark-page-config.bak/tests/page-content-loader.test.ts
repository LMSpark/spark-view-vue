...
import { compileRule, normalizeRuleNode, parseCss, parsePageData, parseScript } from '../src/page-model/read/page-content-compiler'
...
import { PageNodeFilePath } from '../src/page-model/model/page-file/page-file-registry'
import type { RuleConfig } from '../src/page-model/read/page-content-types'
...
describe('PageContentLoader', () => {
  ...
  describe('default remote source', () => {
    ...
    it('loadPageContentConfig: 全部成功 → 组合 PageContentConfig', async () => {
      ...
      const r = await loader.loadPageContentConfig('order-page')
      expect(r.success).toBe(true)
    ...
    it('loadPageContentConfig: 组合结果时间戳取四文件最新后端时间戳', async () => {
      mockFileLoader.load
    ...
    it('loadPageContentConfig: rule 失败 → 快速返回 false', async () => {
      mockFileLoader.load
    ...
    it('loadPageContentConfig: data 失败 → 快速返回 false', async () => {
    ...
    it('loadPageContentConfig: script 非 404 失败 → 返回失败', async () => {
      mockFileLoader.load.mockImplementation((path: string) => {
        if (path === '/order-page/rule.json') return Promise.resolve(fileOk([{ type: 'div' }]))
    ...
    it('loadPageContentConfig: css 非 404 失败 → 返回失败', async () => {
      mockFileLoader.load.mockImplementation((path: string) => {
        if (path === '/order-page/rule.json') return Promise.resolve(fileOk([{ type: 'div' }]))
        if (path === '/order-page/pagedata.json') return Promise.resolve(fileOk({ x: 1 }))
    ...
    it('PageNodeFilePath: 统一编码页面文件缓存 key', () => {
      expect(PageNodeFilePath.forFile('page 1', 'rule.json')).toBe('/page%201/rule.json')
      expect(PageNodeFilePath.forPage('page 1')).toEqual([
        '/page%201/rule.json',
        '/page%201/pagedata.json',
        '/page%201/script.js',
        '/page%201/style.css',
      ])
    })
  ...
  describe('custom apiBaseUrl', () => {
    ...
    beforeEach(() => {
      loader = new PageContentLoader({ apiBaseUrl: '/api' })
    })
    ...
    it('loadPageContentConfig: 远程 rule.json 不存在时 fail-fast', async () => {
      mockFileLoader.load.mockResolvedValueOnce({ success: false, error: 'not found', fromCache: false, reason: 'not-found' })

      const r = await loader.loadPageContentConfig('missing-page')
    ...
    it('loadPageContentConfig: 不请求页面清单，rule.json 缺失由逐文件请求 fail-fast', async () => {
      mockRequestClient.get.mockRejectedValue(new Error('unexpected __list request'))
      mockFileLoader.load.mockResolvedValueOnce({ success: false, error: 'not found', fromCache: false, reason: 'not-found' })

      const r = await loader.loadPageContentConfig('data-report')
    ...
    it('loadPageContentConfig: 远程 pagedata.json 不存在时 fail-fast', async () => {
      mockFileLoader.load
      ...
      const r = await loader.loadPageContentConfig('missing-data-page')
    ...
    it('loadPageContentConfig: 不通过清单跳过 script.js/style.css，脚本缺失 fail-fast', async () => {
      mockRequestClient.get.mockRejectedValue(new Error('unexpected __list request'))
      mockFileLoader.load
        .mockResolvedValueOnce(fileOk([{ type: 'div', id: 'remote-root' }]))
      ...
      const r = await loader.loadPageContentConfig('zero-code-page')
    ...
    it('loadPageContentConfig: style.css 404 会导致整体加载失败', async () => {
      mockRequestClient.get.mockRejectedValue(new Error('unexpected __list request'))
      mockFileLoader.load
        .mockResolvedValueOnce(fileOk([{ type: 'div', id: 'remote-root' }]))
        .mockResolvedValueOnce(fileOk({ dataSetName: 'Empty', tables: {} }))
...
// ────────────────────────────────────────────────────────────────────────────
// 编译 transform 函数单元测试（与 PageContentLoader 解耦）
// ────────────────────────────────────────────────────────────────────────────
