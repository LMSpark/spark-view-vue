...
| **模块级规划** | ①→2→4→5 | “做一个客户管理系统” | → 补权限 → 补 API |
...
app-blueprint.json           （设计时：AI 规划产物）
   ...
   navigation-patch.json      （每页面附属产物，随 AiResponse.files 输出）
        ↓
   PageConfigService.writeBatch（后端钩子：检测到 navigation-patch → 调 addNode）
        ↓
   SSE: navigation-updated    （前端刷新导航树）
```
...
AI 生成页面配置
  ↓ 成功保存
自动调用 NavigationService.addNode()
  ├── pageId → path 映射
...
**实现路径**：
- `AiPageService.processRequest()` 返回结果后，自动调用 `NavigationService.addNode()`
- 在 `PageConfigService.writeBatch()` 中增加导航自动注册钩子
- `SseService` 广播 `navigation-updated` 事件
- 前端接收后刷新导航树
...
| 连接点 | 现状 | 需要做的 |
|--------|------|----------|
| AiPageService → NavigationService | `processRequest()` 返回 files 后**无导航调用** | 在 `_callAI`/`_callAIStream` 文件写入成功后追加 `POST /navigation/nodes` |
| navigation-patch.json 解释器 | `writeBatch` 仅处理 4 种文件 | 新增识别 navigation-patch.json → 自动调用 `addNode` |
| createParentIfMissing 逻辑 | `addNode` 在 parentId 不存在时直接抛异常 | AI 集成层增加先检查/自动创建 parent group 的包装 |
| SSE 导航更新事件 | `SseService` 仅广播页面文件变更 | 增加 `navigation-updated` 事件类型 |

**预估改动量**：约 3 个文件、~100 行代码（AiPageService + PageConfigService + SseService）。
...
**实现路径**：
- 新增 `AiChangeService`（后端）：变更分析 + 增量执行 + 回滚管理
- 新增 `POST /api/ai/change-request`：提交变更请求 → DB 创建 `ChangeRequestEntity` + `ChangeUnitEntity`
- 新增 `POST /api/ai/change-execute`：确认后逐 CU 执行变更计划（DB 状态跟踪）
- 在 `spark-ai` 包新增 `change-planner/` 模块：变更拆解 + 依赖排序 + 增量蓝图生成
...
| `POST /api/ai/page` | POST | 生成单页面配置（现有增强） | ④⑤ |
...
| `POST /api/ai/iterate` | POST | 跨阶段迭代优化 | 全阶段 |
...
### Phase 1：核心增强（短期，1-2 个月）

| 优先级 | 任务 | 涉及阶段 | 工作量 |
|--------|------|---------|--------|
| P0 | **导航自动注册**：AI 生成页面后自动注册导航 | ⑦ | 3 天 |
| P0 | **配置验证器**：跨文件一致性自动检查 | ⑨ | 5 天 |
| P1 | **API 配置生成**：AI 生成 `api-config.json` + DataTable.api 自动映射 | ③ | 7 天 |
| P1 | **声明式交互**：`interactions.json` 解析 + 运行时绑定 | ⑤ | 7 天 |
| P1 | **页面模板库**：内置 8 种模板 + AI 模板选择 | ④ | 5 天 |

### Phase 2：全链路打通（中期，2-3 个月）

| 优先级 | 任务 | 涉及阶段 | 工作量 |
|--------|------|---------|--------|
| P0 | **应用蓝图生成**：`app-blueprint.json` + 模块规划 | ① | 7 天 |
| P0 | **数据库 Schema 生成**：`db-schema.json` + pagedata 自动推导 | ② | 10 天 |
| P0 | **需求变更分析**：change-request.json 影响评估 + 增量策划 | ⑩ | 10 天 |
| P1 | **权限配置生成**：`permission-config.json` + 自动注入 _perm | ⑥ | 7 天 |
| P1 | **多页面编排**：模块级批量生成 + 跨页面一致性 | ⑧ | 10 天 |
| P1 | **分层 Prompt 架构**：按阶段动态组装 system prompt | 全阶段 | 5 天 |

### Phase 3：企业级能力（长期，3-6 个月）

| 优先级 | 任务 | 涉及阶段 | 工作量 |
|--------|------|---------|--------|
| P1 | **版本管理+回滚**：配置快照 + 差异对比 + 一键回滚 | ⑨ | 10 天 |
| P1 | **业务规则引擎**：`business-rules.json` 解析 + 运行时执行 | ⑤ | 14 天 |
| P2 | **全自动模式**：零交互 10 阶段自动编排 | 全阶段 | 7 天 |
| P2 | **变更执行引擎**：DAG 拓扑排序 + 按 CU 回滚 + 回归验证 | ⑩ | 10 天 |
| P2 | **字典管理**：`dictionaries.json` + AI 推断 + 运行时下拉 | ② | 5 天 |
| P2 | **国际化配置**：`i18n.json` AI 生成多语言 | 新增 | 7 天 |
| P3 | **可视化编排**：拖拽编辑器辅助 rule.json 编辑 | ④ | 30 天 |
| P3 | **工作流引擎**：审批流程配置化 | 新增 | 30 天 |
