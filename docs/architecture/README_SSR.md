...
├── src/
│   ├── api/                 # API 请求层
│   ├── pages-config/        # 页面配置与脚本
│   ├── mock/                # Mock 数据层
│   ├── utils/               # 工具函数
│   │   └── page-helpers/    # 通用页面辅助工具
...
1. 在 `src/pages-config/routes.json` 添加路由配置
2. 在 `src/pages-config/<pageName>/` 创建页面配置：
   - `rule.json` - 组件规则
   - `pagedata.json` - 页面数据元数据
   - `style.css` - 页面样式
3. 在 `src/pages-config/<pageName>/script.js` 创建页面脚本
