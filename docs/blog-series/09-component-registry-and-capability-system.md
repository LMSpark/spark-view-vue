...
在 PageDesign 这个业务样例里，AI 不能靠猜测组件名完成页面编辑。组件注册和 catalog 构成基础事实，而 PageDesign 业务模块再把这些事实投影为组件参数荷载指南。注意这里的归属：核心层只提供 AI Core 协议和注册机制；`PageDesignComponentPayloadProvider`、`queryPayloads`、`guidePayload` 属于 PageDesign 业务模块。
