...
SPARK View 是企业后台的软件项目模型与页面运行平台。它不是 JSON 表单生成器，也不是让 AI 任意生成 Vue 代码的工具。它把项目需求、模块规划、页面功能、结构、数据、脚本、样式和权限收敛到可验证的模型链路。
...
| `spark-page-config` | ProjectModel、ProjectNode、项目策划、配置页节点子模型 |
...
它可以编辑、预览、诊断、管理版本和启动 AI，但不能直接绕过 `spark-page-config/project` 操作后端节点、页面文件或跨项目引用。
