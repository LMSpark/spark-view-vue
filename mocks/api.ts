...
import routes from '../public/pages-config/routes.json'
import fs from 'fs'
...
const loadPageConfig = (pageId: string, type: 'rule' | 'data' | 'style') => {
  try {
    ...
    if (type === 'rule' || type === 'data') {
      ...
      const config = require(`../public/pages-config/${pageId}/${fileName}.json`)
    ...
    if (type === 'style') {
      try {
        const stylePath = path.join(__dirname, `../public/pages-config/${pageId}/style.css`)
