...
Spark AI agent runtime：JSON Schema 校验、VCM 模块协议、`AiModuleRuntime`、Host 编排、ToolLoop、传输契约与 native-runtime 脚本执行。
...
| `@spark-appworks/spark-ai/modules` | 模块协议、`AiModuleRuntime`、metadata 图 |
| `@spark-appworks/spark-ai/agent` | Host、会话、ToolLoop、传输、native-runtime |
| `@spark-appworks/spark-ai/vcm-native` | ClassModel 反射图与 VCM-native 工具（演进中） |
