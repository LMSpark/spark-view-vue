...
SPARK 的 AI 运行时包，采用业务注册架构：核心运行时负责 LLM 适配会话、函数曝光与调用分发，业务层负责 page-design 编辑服务与具体领域能力。

## 主要职责

- `core`：业务注册、适配会话生命周期、`core@knowledge` 只读模型、函数调用与历史事件
...
- `@spark-view/spark-ai`：兼容聚合入口（同时导出 core/business/catalog）
