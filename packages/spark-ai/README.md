...
- `core`：递归模块注册、AI 会话历史、LLM 知识投影、函数调用翻译、函数结果回传协议、知识负载提供者注册表。
- `business`：`page-design` 四文件编辑模块（`rule.json`、`pagedata.json`、`script.js`、`style.css`）、模块自有状态和真实函数执行。
- `catalog`：组件目录投影与 DevSystem 预计算元数据。

## 核心层语义

- **统一叫模块**：不再区分业务、根模块、子模块；模块通过 `modules` 递归形成树。
- **目录元数据**：模块目录只描述本模块的 `functionId` 和子模块 `modules`；目录不描述调用路径。
- **函数路径**：LLM-facing action 由 core 按当前 AI 会话投影生成，格式为 `rootInstanceId[/childInstanceId]@moduleId@functionId`，例如 `page-designer@nodeTree@addNode`。
- **实例路径编码**：action 中的实例路径段会 URI 编码后再投影，例如页面 ID `lmspark/homepage` 会变成 `lmspark%2Fhomepage@...`，core 翻译时还原真实业务 ID。
- **中心链路**：业务模块实体先通过注册把知识交给 core；之后 LLM 的函数编排必须经过 core session，再回到业务模块实体执行。
- **注册句柄**：`registerModule` 返回绑定 `moduleId` 的 `AiRegisteredModuleApi`，注册方用它串起会话开始、知识投影、消息记录、函数调用记录、翻译、结果回传和会话停止。
- **隔离边界**：AI 会话以 `moduleId + moduleInstanceId` 隔离，也就是“业务能力注册 ID + 根业务模块实体 ID”；`instanceId` 只是技术 envelope/alias。
...
- **模块实例归属**：`moduleInstanceId`、active path、业务状态和服务释放都由注册方或会话宿主管理。
...
- `@spark-view/spark-ai/core`：核心协议、知识投影、函数调用翻译与知识负载提供者注册表。
