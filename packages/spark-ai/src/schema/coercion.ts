...
import type { LlmJsonValue } from './types'
...
export function coerceJsonValue(value: unknown): LlmJsonValue | undefined {
  return coerceInternal(value, new WeakSet<object>())
}

export function coerceStrictJsonValue(value: unknown): LlmJsonValue | undefined {
...
function coerceInternal(value: unknown, seen: WeakSet<object>): LlmJsonValue | undefined {
...
function coerceStrict(value: unknown, seen: WeakSet<object>): LlmJsonValue | typeof STRICT_REJECT | undefined {
  if (value === null) return null
  if (value === undefined) return undefined
  if (typeof value === 'string') return value
...
function withCycleGuard<TValue extends LlmJsonValue>(
  value: object,
  seen: WeakSet<object>,
  createValue: () => TValue,
): TValue | undefined {
...
function withCycleGuardStrict<TValue extends LlmJsonValue>(
  value: object,
  seen: WeakSet<object>,
...
function coerceIterable(items: Iterable<unknown>, seen: WeakSet<object>): LlmJsonValue[] {
...
function coerceIterableStrict(items: Iterable<unknown>, seen: WeakSet<object>): LlmJsonValue[] | typeof STRICT_REJECT {
  const out: LlmJsonValue[] = []
  for (const item of items) {
...
function coerceRecordStrict(
  entries: Iterable<readonly [unknown, unknown]>,
  seen: WeakSet<object>,
): Record<string, LlmJsonValue> | typeof STRICT_REJECT {
  const out: Record<string, LlmJsonValue> = {}
  for (const [key, raw] of entries) {
