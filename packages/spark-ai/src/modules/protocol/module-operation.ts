 ...
 *   一、诊断条目 — AiModuleCheck
 *   二、操作结果 — AiModuleResult<TData>
 *   三、内部辅助
 */
...
export type AiModuleCheckLevel = 'error' | 'warn' | 'info'

export class AiModuleCheck {
  /**
   * level — 严重级别（error / warn / info）
   * code  — 错误码（机器可读）
   * message — 人类可读描述
   ...
   */
  public constructor(
    public readonly level: AiModuleCheckLevel,
  ...
  public static error(code: string, message: string, hint?: string): AiModuleCheck {
    return new AiModuleCheck('error', code, message, hint)
  }

  /** warn 级：操作成功但存在值得关注的问题。不阻断流程，但提示 LLM 注意。 */
  public static warn(code: string, message: string, hint?: string): AiModuleCheck {
    return new AiModuleCheck('warn', code, message, hint)
  }

  /** info 级：纯信息性提示。告知 LLM 操作上下文，不影响决策。 */
  public static info(code: string, message: string, hint?: string): AiModuleCheck {
    return new AiModuleCheck('info', code, message, hint)
  }
}
...
//   TData=AiJsonValue                  → getAttribute / 业务函数调用
//   TData=readonly AiModuleInstanceRef[]  → listChildren / findInstance
//
// 关键约束：
...
//
// 构造流程：AiModuleResultOptions → 构造函数（自动过滤空 checks） → 实例
// ============================================================================

export type AiModuleResultOptions<TData> = Readonly<{
  ok: boolean
  data?: TData
  checks?: readonly AiModuleCheck[]
  state?: Record<string, unknown>
}>

export class AiModuleResult<TData = unknown> {
  /** 操作是否成功 */
  public readonly ok: boolean
  /** 成功时的业务数据（失败时为 undefined） */
  public readonly data?: TData
  /** 诊断条目列表（失败时至少含一条 error；空数组等价于 undefined） */
  public readonly checks?: readonly AiModuleCheck[]
  /** 附加状态数据（透传用，如请求 ID、时间戳等） */
  public readonly state?: Record<string, unknown>

  public constructor(options: AiModuleResultOptions<TData>) {
    this.ok = options.ok
    if ('data' in options) {
  ...
  public static fail(
    ...
    if (checks.length === 0) {
      throw new Error('AiModuleResult.fail requires at least one AiModuleCheck')
    }
    return new AiModuleResult({
      ok: false,
      checks,
  ...
  public static failCode(code: string, message: string, hint?: string): AiModuleResult<never> {
    return AiModuleResult.fail([AiModuleCheck.error(code, message, hint)])
  }

  /**
   * 透传上游失败结果。保留原始 checks + state，不丢失错误链信息。
   * 典型场景：AiModule 协议方法调用委托返回失败时，原样向上传递。
   */
  public static passthroughFailure(result: AiModuleResult<unknown>): AiModuleResult<never> {
    return new AiModuleResult({
      ok: false,
...
function nonEmptyChecks(checks: readonly AiModuleCheck[] | undefined): readonly AiModuleCheck[] | undefined {
