...
export function payloadRefsForFunction(
  payloads: readonly AiModulePayloadMetadata[] | undefined,
  functionName: string,
): readonly string[] {
  return (payloads ?? [])
    .filter((payload) => {
...
export function summarizeFunction(options: FunctionKnowledgeProjectionOptions): AiModuleKnowledgeFunctionSummary {
  ...
  return {
    ...
    resultApiKinds: uniqueTexts((fn.resultApis ?? []).map(resultApi => resultApi.kind)),
...
export function createGuide(options: FunctionKnowledgeProjectionOptions): AiModuleKnowledgeFunctionGuide {
  ...
  return {
    ...
    scriptPattern: {
      receiver: 'this',
      call: `await this.call("${fn.name}", { path, args })`,
      args: 'object matching paramsSchema; construct it from this guide before executing sandbox script',
    },
    programmingFlow: [
