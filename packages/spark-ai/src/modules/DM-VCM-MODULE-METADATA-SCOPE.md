...
VCM 的目标是替代手写 `AiModule`：构建期从领域能力 class 源码、JSDoc、构造函数和能力模块边界中提取语义元数据，生成标准 `AiModuleFunctionMetadata` / `AiModuleAttributeMetadata` 输入和 `AiModule` factory，再由生成代码创建 `AiModule` 并调用 `runtime.register`。

进入协议层后的标准形态仍然只有：

- `AiModule.kind / name / description`
- `AiModule.attributes`
- `AiModule.functions`
- `AiModule.children`
- 构造期运行委托：`runner / list / find` 输入，只能通过 `AiModule` 协议方法访问
...
手写 `AiModule` class 只作为迁移期兼容形态存在；新增能力模块不应把元数据硬编码在自定义 `AiModule` 子类里。业务代码只保留构造期运行委托、构造依赖和服务实现，语义元数据从能力模块本体提取，注册层只承接装配边界。
...
import {
  AiModule,
  type AiModuleChildrenLister,
  type AiModuleInstanceFinder,
  type AiModuleRunner,
...
export function createNodeTreeAiModule(delegates: {
  readonly runner: AiModuleRunner
  readonly find?: AiModuleInstanceFinder
  readonly list?: AiModuleChildrenLister
}): AiModule {
  return new AiModule({
...
运行时仍只认 `runtime.register(...)` 这一个注册口。VCM 可以生成 `AiModule` subclass 构造器或 factory；构造器由 `register(Constructor, ...args)` 创建实例，factory 的返回值必须是标准 `AiModule`。

VCM 应完整提取六类结构：

- **属性**：生成 `AiModuleAttributeMetadata[]`，包括读写能力、值 schema、示例和属性语义。
- **动作**：生成 `AiModuleFunctionMetadata[]`，包括 `paramsSchema / resultSchema / usageRules / failureModes / example`。
...
- **运行委托**：按 JSDoc 标识提取 `attributeAccessor/runner/list/find` 委托函数，把它们作为 `AiModule` 构造参数接入；运行期只能通过 `getAttribute/setAttribute/标准 function tool/listChildren/findInstance` 协议方法访问。VCM 只识别委托签名和绑定关系，不解析委托函数体来推导业务语义。
- **攻击面**：按 JSDoc 标识提取能力模块能触达的资源、信任边界、写入/删除/执行风险和防护规则。VCM 只提取声明，不从业务实现反推安全语义。

## JSDoc 标识

VCM 提取能力模块元数据时，能力语义只识别写在领域能力 class 源码附近的 JSDoc，例如 `SparkNodeTree`、`DataSetCrudTool` 这类真正承载动作和状态不变量的 class。`assistant/registrations/**` 这类注册源只允许声明 factory、attributeAccessor/runner/list/find 委托和依赖装配；不得承载实体、攻击面、业务动作语义。迁移期可以读取已有 `AiModule` class 上的装配注释用于回填；目标形态不要求业务继续手写 `AiModule` class。
...
- `@moduleKind <kind>`：声明目标 `AiModule.kind`，必须与注册时的 `kind` 一致。
- `@moduleName <name>`：声明给 LLM 看的能力模块名称。
- `@moduleDescription <text>`：声明能力模块描述。可省略；省略时以 `AiModule.description` 为准。
...
- `@moduleFactory <factoryName>`：声明生成 factory 名称；返回值必须是标准 `AiModule`。
- `@moduleRunner <functionName>`：声明 function runner 委托。目标函数必须满足 `AiModuleRunner`，或由生成 factory 包一层后满足该签名。
- `@moduleListDelegate <functionName>`：声明 list 委托。目标函数必须满足 `AiModuleChildrenLister`。
- `@moduleFindDelegate <functionName>`：声明 find 委托。目标函数必须满足 `AiModuleInstanceFinder`。
...
函数与属性仍以现有 `AiModuleFunctionMetadata` / `AiModuleAttributeMetadata` 为最终协议形态：

- function 的参数、返回、规则、失败模式只进入 `paramsSchema / resultSchema / usageRules / failureModes / example`。
- attribute 的读写语义只进入 `AiModuleAttributeMetadata`。
- JSDoc 只用于 VCM 生成和一致性检查，不新增运行时协议字段。
- 参数说明复用标准 JSDoc `@param`；返回说明复用 `@returns` / `@return`。JSON Schema 可以由类型系统、`.dm` 约束和显式 schema 常量共同生成，但最终必须落到 `AiJsonSchemaObject`。
 ...
 * @moduleDependency contextFactory (ctx: AiModulePathContext) => PageDesignServiceContext
 * @moduleFactory createNodeTreeAiModule
 * @moduleRunner runNodeTreeAction
 * @moduleFindDelegate findCurrentNodeTree
 */
export function createNodeTreeAiModule(): AiModule {
  // 注册层只负责把能力模块元数据和运行委托投影为 AiModule。
}
```
...
- `AiModuleFunctionMetadata` / `AiModuleAttributeMetadata` 常量，以及它们被传入 `AiModule` 的标准 metadata。
- 与 `AiModule` 发现链路直接相关的 `children`、`list`、`find` 语义说明。
...
- `@moduleKind` 与 `AiModule.kind` 不一致时 fail-fast。
- `@moduleChild` 与 `children` 不一致时 fail-fast。
- JSDoc 与 `AiModuleFunctionMetadata` / `AiModuleAttributeMetadata` 冲突时，以标准 schema 为准并报诊断，不做静默覆盖。
...
- VCM 生成的 metadata 必须在 `new AiModule(...)` factory 调用处通过 TypeScript 校验。
...
- 最终注册仍必须走 `runtime.register(new AiModule(...))`、`runtime.register(AiModuleCtor, ...args)` 或等价的生成 factory 返回值；不得新增绕过 `AiModule` 的 runtime 注册入口。

## 术语迁移延后边界（2026-05-27）

**本轮实际范围**：`packages/spark-ai/src/modules`（protocol 层）和 `packages/spark-ai/src/agent`（transport/chat/session 层）已完成 `action`→`function` 术语统一。包括 protocol 源码、类型名、注释、工具描述和 spark-ai 测试文件。

**延后区域**：以下属于业务层/VCM 实现层，不在本轮迁移范围：

- `packages/spark-page-config/src/ai/*-tool-catalog.ts` — 内部 `actionName` 参数名和 `runAction` 方法名是 catalog class 的私有 API，与 AiModule 协议无耦合。
...
`UNKNOWN_ACTION` 错误码保留不变——它是 `AiModule` 运行时的稳定契约字符串，已是现有业务方的错误匹配锚点，改名为 `UNKNOWN_FUNCTION` 属于不必要的 breaking change。
