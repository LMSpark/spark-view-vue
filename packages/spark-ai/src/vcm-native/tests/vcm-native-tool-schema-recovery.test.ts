...
describe('buildVcmNativeToolSchemaRecoveryHint', () => {
  ...
  it('derives vcm_model_guide required className from native tool schema', () => {
    const hint = buildVcmNativeToolSchemaRecoveryHint(VCM_NATIVE_TOOL_NAMES.modelGuide)
    expect(hint).toContain('className (required')
    expect(hint).not.toContain('modelName')
  })

  it('derives vcm_attribute_guide className and attributeName from native tool schema', () => {
