import { findVcmNativeToolSpec } from './vcm-native-tool-specs'

/** 从 VCM-native 工具闭集 schema 派生参数恢复提示，避免业务层硬编码协议字段名。 */
export function buildVcmNativeToolSchemaRecoveryHint(toolName: string): string | undefined {
  const spec = findVcmNativeToolSpec(toolName)
