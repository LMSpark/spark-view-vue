 ...
 *
 * 该模块只解释 `AiHostSessionRecord` 的通用历史结构，用于 smoke、调试面板或日志导出。
 * 它不理解 pageDesign、业务模块、工具参数语义，也不改变 sessionStore 状态。
 */
...
// PAGE_DESIGN_REFACTOR_SOURCE[session-diagnostics]: AI 会话历史摘要/转录的通用来源；smoke 只读取，不自己解析 sessionRecord 结构。
/**
 * 汇总会话历史的数量、失败工具调用和最后一条助手文本。
 *
 * `sessionRecord` 为空时返回空摘要，便于 smoke 在会话启动失败后仍能生成诊断 JSON。
 */
 ...
 *
 * 转录会裁剪长字段，但不会丢弃条目顺序；调用方可直接写入 smoke JSON 或调试面板。
 */
