...
type AiHostToolNameResolver = (toolName: string) => string | null

/** 工具调用执行的输入参数 */
type AiHostToolCallExecutionInput<TInput extends LlmJsonParams = LlmJsonParams> = Readonly<{
  registration: AiHostBusinessRegistration<TInput>
  scope: AiHostBusinessScope
  turn: AiHostTurnMeta
  round: number
  /** transport toolName → runtime toolName 的解析函数 */
  resolveToolName: AiHostToolNameResolver
...
/** 工具调用执行的输出 */
type AiHostToolCallExecutionOutput = Readonly<{
