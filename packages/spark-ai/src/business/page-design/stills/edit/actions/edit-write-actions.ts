...
import { TEXT_MODEL_STILLS_PARAMETER_TABLE } from '../../text-model'
...
export function isEditDataSetWriteAction(action: string): boolean {
  return DATASET_CRUD_TOOL_STILLS_PARAMETER_TABLE.some(
...
export function isEditTextModelWriteAction(action: string): boolean {
  return TEXT_MODEL_STILLS_PARAMETER_TABLE.some((row) => row.type === 'request' && row.action === action)
}
