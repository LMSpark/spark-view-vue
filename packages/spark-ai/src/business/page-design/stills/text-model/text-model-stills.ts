...
import {
  ...
  type TextModelStillParameterRow,
} from './tool-catalog'
...
type FileRuntime = {
  ...
  validateWrite?: (content: string) => StillResult<undefined> | null
}
...
function ensureTextModelAccess(
  ...
  const host = getEditState(session).toolHost
...
function noTextModelResult(msg: string): StillResult<undefined> {
  return {
    ok: false,
    code: 'NO_TEXT_MODEL',
    msg,
...
function createTextModelStill(row: TextModelStillParameterRow): StillDefinition {
  ...
  return {
    ...
    validate: (params) => validateTextModelStillParams(row.action, params),
    execute: (session, params): StillResult => {
      const accessError = ensureTextModelAccess(session, row.fileKey, mode)
      if (accessError) {
...
export const EDIT_FILE_STILLS: StillDefinition[] = TEXT_MODEL_STILLS_PARAMETER_TABLE.map(createTextModelStill)
