...
import {
  ...
  type PageDesignJsonDocOperation,
...
export type JsonDocFunctionId = PageDesignJsonDocOperation
...
export type JsonDocFunctionCapabilityRow = Pick<
  ...
  'functionId' | 'type' | 'target' | 'description' | 'operation'
> & {
...
const BOOTSTRAP_RULE = '调用 jsonDoc 函数前必须先完成 lifecycle.bootstrap，确保宿主绑定 readJsonDoc/writeJsonDoc。'
const POINTER_FORMAT_RULE = 'pointer 遵循 RFC 6901：以 "/" 开头（"" 代表文档根）；段间以 "/" 分隔；"/" 用 ~1 转义，"~" 用 ~0 转义。'
const IMMUTABLE_RULE = 'set/delete/append/setMultiple 不直接修改原始 JSON 文件，所有写入通过 PageDesignEditHost.writeJsonDoc 提交并触发宿主持久化。'
...
const NO_JSON_DOC_HOST: JsonDocFunctionFailureMode = {
  ...
  when: '宿主未绑定 PageDesignEditHost.readJsonDoc / writeJsonDoc',
  fix: '先执行 lifecycle.bootstrap 并确保宿主提供 readJsonDoc/writeJsonDoc。',
}
...
type JsonDocFunctionRowWithoutType = Omit<JsonDocFunctionParameterRow, 'type' | 'runtimeBinding' | 'runtimeRegistration'>

const defineDescribeRow = (row: JsonDocFunctionRowWithoutType): JsonDocFunctionParameterRow => ({
  type: 'describe',
  runtimeBinding: {
    kind: 'page-design-service',
    method: 'useJsonDocOperation',
    jsonDocOperation: row.operation,
  },
  runtimeRegistration: 'registered',
  ...row,
})
...
> {
  ...
  override validateParams(functionId: string, params: unknown): string | null {
    const row = this.getParameterRow(functionId as JsonDocFunctionId)
    if (row === undefined) return `未知 jsonDoc 函数: ${functionId}`
    if (params === null || typeof params !== 'object' || Array.isArray(params)) {
      return `${functionId}: params 必须为对象`
    }
    ...
    if (p['docType'] !== 'pagedata' && p['docType'] !== 'rule') {
      return `${functionId}: docType 必须是 "pagedata" 或 "rule"`
    }

    if (row.operation === 'list' || row.operation === 'get') {
      if (typeof p['pointer'] !== 'string') return `${functionId}: pointer 必须是 string`
    }

    if (row.operation === 'set') {
      if (typeof p['pointer'] !== 'string') return `${functionId}: pointer 必须是 string`
      if (!('value' in p)) return `${functionId}: 缺少 value`
    }
    ...
    if (row.operation === 'append') {
      if (typeof p['arrayPointer'] !== 'string') return `${functionId}: arrayPointer 必须是 string`
      if (!('element' in p)) return `${functionId}: 缺少 element`
    }

    if (row.operation === 'setMultiple') {
      if (!Array.isArray(p['patches'])) return `${functionId}: patches 必须是数组`
      for (let i = 0; i < (p['patches'] as unknown[]).length; i++) {
        const patch = (p['patches'] as unknown[])[i]
        if (patch === null || typeof patch !== 'object' || Array.isArray(patch)) return `${functionId}: patches[${i}] 必须是对象`
        const pObj = patch as Record<string, unknown>
        if (typeof pObj['pointer'] !== 'string') return `${functionId}: patches[${i}].pointer 必须是 string`
        if (!('value' in pObj)) return `${functionId}: patches[${i}] 缺少 value`
      }
    }

    if (row.operation === 'query') {
      if (typeof p['expression'] !== 'string') return `${functionId}: expression 必须是 string`
      if ('pointer' in p && typeof p['pointer'] !== 'string') return `${functionId}: pointer 若提供则必须是 string`
    }
