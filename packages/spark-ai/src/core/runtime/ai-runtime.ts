 ...
 *
 * 语义中心是 AI session：
 * - A：业务模块实体保存自己的业务状态和执行器。
 * - C：业务模块注册把模块知识与函数知识交给 core。
 * - B：core 会话管理保存 session/history，并把知识投影给 LLM。
 * - LLM：在会话上下文中编排函数调用。
 * - B：core 记录 requested 调用、翻译调用、回填 completed/failed 结果。
 * - A：业务模块实体只在 B 翻译后执行真实函数。
 *
 * start/stop 与 AI 会话生命周期一致：核心会保存 AI session 状态和
 * UI/LLM/函数调用历史；但它不创建模块服务实例、不释放模块、不保存
 * 业务状态，也不调用函数体，更不会根据执行结果决定下一步调用。
 */
export class AiRuntime implements AiRuntimeApi {
  ...
  /** AI 会话表。主键是 `moduleId + moduleInstanceId`，即业务能力注册 ID + 根业务模块实体 ID。 */
  private readonly sessions = new Map<string, AiRuntimeSessionRecord>()

  /** 技术 instanceId 到业务隔离键的兼容索引；instanceId 不决定 session 隔离。 */
  private readonly sessionScopesByInstanceId = new Map<string, string>()

  /** 全局单调递增历史序号。序号只用于 core ledger，不参与业务排序。 */
  ...
  /** 按业务能力注册 ID + 根业务模块实体 ID 读取 AI 会话记录。 */
  ...
  /** 按业务隔离键读取 AI 会话历史；这是模块注册方更稳定的读取入口。 */
  ...
  async translateFunctionCall(
    ...
    try {
      ...
      return AiRuntime.createFailure('INVALID_ACTION', AiInvocationProtocol.toErrorMessage(error), 'Use action format rootInstance[/childInstance]@module@function.')
    }
    ...
    if (address.format === 'instance' && address.instanceIds[0] !== session.moduleInstanceId) {
      return AiRuntime.createFailure(
        ...
        'Use the root business entity id from the current session as the action prefix.',
      )
    }
  ...
  /** 创建或更新 Started session。这里保存 AI 会话状态，不触碰模块业务状态。 */
  ...
  /** translate 类 API 使用：按业务隔离键检查 session，并把状态问题转换成 LLM/tool 友好的结构化失败。 */
  ...
  /** 保存 AI session，并维护技术 instanceId 到业务隔离键的兼容索引。 */
  ...
  /** 防止同一个 AI session id 被复用到另一个业务模块实例上。 */
  ...
  /** 创建注册方使用的模块绑定 API；只补齐 moduleId，不持有业务服务实例。 */
  ...
  /** start 会保留同一业务实体的历史，但允许调用方切换新的技术 envelope alias。 */
  ...
  /** 确保同一个技术 alias 不会被复用到另一个业务隔离键，避免 session 查询串线。 */
  ...
  /** 把 instanceId/runtimeInstanceId 作为兼容 alias 绑定到同一业务隔离键。 */
  ...
  private findFunctionExposure(
    ...
    const found = functions.filter((candidate) => (
      ...
      && candidate.functionId === address.function
    ))
    if (found.length > 1) {
  ...
  private validateActionInstancePath(
    ...
    if (address.instanceIds[0] !== scope.moduleInstanceId) {
      return AiRuntime.createFailure(
        ...
        'Use the root business entity id from the current AI session.',
      )
    }
    return null
  }
  ...
  /** 校验普通 ID 非空；业务实例 ID 允许包含分隔符，进入 action 时会被 URI 编码。 */
  ...
  /** 把业务实例 ID 编码成 action 路径段，避免 `/` 或 `@` 破坏 LLM 调用地址。 */
  ...
  /** 将任意执行结果转为 tool result 字符串；这里只处理序列化，不解释业务含义。 */
