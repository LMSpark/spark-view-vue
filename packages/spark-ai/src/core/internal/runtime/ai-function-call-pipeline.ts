...
export class AiFunctionCallPipeline {
  constructor(
  ...
  // ── Private: translation (formerly AiFunctionCallTranslator) ──

  private async translate(
    options: AiRuntimeTranslateFunctionCallOptions,
  ): Promise<{ ok: true; translation: AiRuntimeFunctionCallTranslation } | AiRuntimeFunctionCallFailure> {
    ...
    const validationError = LlmParamsValidator.validateLlmDeserializedParams(contextArgs.effectiveArgs, exposure.paramsSchema)
    if (!validationError.ok) {
      return createFunctionCallFailure('INVALID_ARGS', LlmParamsValidator.formatLlmParamValidationIssues(validationError.issues), `Use paramsSchema from projection.availableFunctions for ${options.action}.`)
    }
  ...
  private prepareExecutionArgs(
    ...
    return { effectiveArgs, executionArgs, moduleInstances }
  }
  ...
  private normalizeActivePathBinding(module: AiRuntimeModuleExposure, binding: AiModuleInstanceBinding): AiModuleInstanceBinding {
    ...
    return { modulePath: binding.modulePath, instanceId: binding.instanceId, paramName }
  }
  ...
  private mergeActionInstancePath(
    ...
    for (let index = 0; index < contextParams.length; index++) {
      ...
      bindings.push({ modulePath: param.modulePath, paramName: param.paramName, instanceId })
    }
  ...
  // ── Private: execution helpers (formerly AiFunctionCallExecutor) ──
  ...
  private completeTranslatedFunctionCall(
    ...
    const resultMessage = this.createFunctionResultMessage({ action: translation.action, result })
