 ...
 * LLM 函数调用翻译结果，以及把注册方执行结果转换成 LLM 可消费消息的最薄协议。
 ...
 * - core 不保存模块业务状态，也不维护 active path 业务状态。
 * - core 不执行函数体，不读取执行结果做编排、重试、轮次推进。
 * - core 不校验函数执行结果；结果只会被原样序列化后回传给 LLM。
 * - 模块服务生命周期、业务状态和函数派发由注册方或会话宿主管理。
 *
 * 语义主链路：
 * A 业务模块实体 -> C 业务模块注册 -> B core 会话管理 -> LLM 函数编排
 * -> B core 记录/翻译/回传 -> A 业务模块实体执行。
 *
 * 因此，B 是 AI core 的中心：它拥有 AI 会话轨迹，却不拥有 A 的业务状态。
 */
...
/** 注册方/宿主管理的模块实例 ID，通常对应当前 AI 会话绑定的业务实例。 */
...
/** LLM 调用函数时使用的完整路径地址，例如 `rootId/childId@basicInfo@update`；实例路径段按 URI 编码。 */
...
export interface AiFunctionRegistration {
  ...
  /** 面向 LLM 的结果 schema；仅用于说明，不由 core 校验业务结果。 */
  readonly resultSchema?: Record<string, unknown> | undefined
  /** 建议的最长执行时间；core 不执行函数，因此只透传给上层。 */
...
export interface AiModuleRegistration {
  /** 模块 ID；同一注册树内必须唯一，因为 LLM action 使用 `@moduleId@functionId` 定位函数。 */
  readonly moduleId: AiRuntimeModuleId
  /** 模块名称。 */
  readonly name: string
  /** 模块职责说明。 */
...
export interface AiRuntimeFunctionExposure {
  /** core 基于当前会话投影出的完整 LLM action，例如 `page-1@nodeTree@addNode`。 */
  readonly action: AiRuntimeAction
  /** 函数所在模块 ID。 */
  readonly moduleId: AiRuntimeModuleId
  /** 函数所在模块路径。 */
 ...
 * 对话消息角色。
 *
 * 这里记录的是 AI 会话中进入模型上下文的消息角色，不等同于业务模块角色。
 */
...
export interface AiRuntimeSessionRecord extends AiRuntimeInstanceScope {
  ...
  readonly moduleId: AiRuntimeModuleId
  /** 根业务模块实体 ID；与 moduleId 一起隔离 AI 会话。 */
  readonly moduleInstanceId: AiRuntimeModuleInstanceId
...
export interface AiRuntimeOptions {
  /** 可注入时间源，仅用于会话生命周期通知快照；不参与业务生命周期决策。 */
  now?: () => number
}
 ...
 * 它不创建、停止或释放业务服务实例；注册方可在自己的服务管理代码里组合这些 API。
 */
...
export interface AiRuntimeApi {
  ...
  getSession(instanceId: string): AiRuntimeSessionRecord | null
  /** 按业务能力注册 ID + 根业务模块实体 ID 读取 AI 会话记录；未知时返回 null。 */
