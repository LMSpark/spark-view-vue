...
type TaskRegistrationHooks = Readonly<{
  beforeFunctionCall?: NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['beforeFunctionCall']>
  afterFunctionCall?: NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['afterFunctionCall']>
  onEndBusinessInstance?: NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['onEndBusinessInstance']>
}>
type TaskRequestBeforeFunctionCall = NonNullable<AiAgentTaskChatOptions['beforeFunctionCall']>

class TaskApi {
  public complete(): AiModuleResult<AiJsonValue> {
    return AiModuleResult.ok({ done: true })
  }

  public fail(): AiModuleResult<AiJsonValue> {
    return AiModuleResult.failCode('INTENTIONAL_FAILURE', '工具失败', '读取 diagnostics 后修正参数或业务状态。')
  }
}

const TASK_METADATA: AiModuleMetadataJson = {
  schemaVersion: 1,
  rootApi: {
    kind: 'task',
    name: '任务',
...
const TICKET_METADATA: AiModuleMetadataJson = {
  schemaVersion: 1,
  rootApi: {
    kind: 'ticket',
    name: '工单',
    ...
    actions: [
      {
        name: 'noop',
        methodName: 'noop',
        description: '占位 action，供 inspect 通过',
        paramsSchema: noParamsSchema(),
      },
    ],
  },
...
class TicketApi {
  public noop(): AiModuleResult<AiJsonValue> {
    return AiModuleResult.ok(null)
  }
}
...
function createRegistration(
  ...
  return AiModuleAdapter.createRegistration({
    ...
    options: {
      ...
      sessionStore: store,
      inputContract: createSimpleInputContract<TaskInput>({
        businessId: 'task',
...
describe('AiAgentHost public API', () => {
  ...
  it('creates adapter registrations and dry-runs without calling LLM', () => {
    ...
    expect(registration.runtime.inspect()).toMatchObject({ ok: true, rootKinds: ['ticket'] })
  ...
  it('rejects a tool call in beforeFunctionCall without executing runtime or after hook', async () => {
    const store = new DefaultAiAgentSessionStore()
    const beforeFunctionCall = vi.fn<NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['beforeFunctionCall']>>(() => ({
      status: 'reject',
      reason: '需要人工审批',
      fix: '等待用户批准后再执行。',
    }))
    const afterFunctionCall = vi.fn<NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['afterFunctionCall']>>(() => ({
      status: 'continue',
    }))
    ...
    expect(beforeFunctionCall).toHaveBeenCalledWith(
      expect.any(TaskApi),
      expect.objectContaining({
        moduleId: 'task',
  ...
  it('aborts the session when beforeFunctionCall returns abort', async () => {
    const store = new DefaultAiAgentSessionStore()
    const onEndBusinessInstance = vi.fn<NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['onEndBusinessInstance']>>()
    const beforeFunctionCall = vi.fn<NonNullable<AiModuleAdapterRegisterOptions<TaskApi>['beforeFunctionCall']>>(() => ({
      status: 'abort',
      reason: '用户取消工具执行',
    ...
    expect(onEndBusinessInstance).toHaveBeenCalledWith(
      expect.any(TaskApi),
      expect.objectContaining({
        moduleId: 'task',
        moduleInstanceId: 'task-a',
