 ...
 *
 * 【设计决策】
 *   - 每个构造器返回标准 AiJsonSchemaObject，避免手写时遗漏 type/properties。
 *   - 所有函数都是纯工厂，无副作用、无状态。
...
export function anySchema(description?: string): AiJsonSchemaObject {
  return {
...
export function numberSchema(description: string, options: NumberSchemaOptions = {}): AiJsonSchemaObject {
  return {
...
export function booleanSchema(description: string, options: BooleanSchemaOptions = {}): AiJsonSchemaObject {
  return {
...
export function arraySchema(items: AiJsonSchema = anySchema(), description?: string): AiJsonSchemaObject {
  return {
    type: 'array',
    items,
    ...(description !== undefined ? { description } : {}),
...
export type ObjectSchemaOptions = Readonly<{
  ...
  description?: string
  additionalProperties?: AiJsonSchema
}>
 ...
 */
export function noParamsSchema(description = '不接受参数，请传 {} 或留空。'): AiJsonSchemaObject {
  return objectSchema({}, {
