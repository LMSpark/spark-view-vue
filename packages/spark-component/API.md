...
| 键 | 类型 | 说明 |
|---|---|---|
| `PAGE_RUNTIME_SERVICES` | `PageRuntimeServicesCapability`，来自 `@spark-view/spark-page-config/runtime` | `{ router?, logger?, tenant?, configLoader?, authService?, pageService? }` |
...
自解析 `dataViewKey`，提供 `DATA_SOURCE` 能力；展示组件读取 DataView 输出时使用 `dataViewKey + dataMember + dataField`。
