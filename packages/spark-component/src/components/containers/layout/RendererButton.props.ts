...
export type RButtonProps = SparkNodeProps & {
   ...
   * @description 新建申请、提交新单据等“创建一条新记录”的按钮优先使用 append-row，并用 dataViewKey 指向目标列表/待处理视图。
   * @binding append-row/prompt-append 会读取按钮当前作用域行、appendPayload、inheritFields 和 inheritFieldMap。
   ...
   * @description append-row/patch/delete/refresh 等动作会优先作用于这个 DataView。
   * @binding table@viewId
   */
  dataViewKey?: string
   ...
   * @description 适合写入固定状态，例如 {"status":"pending"}。
   * @default {}
   */
   ...
   * @description 表单提交到待处理列表时，用它把 currentRow 中的申请人、日期、事由等字段复制到目标视图。
   * @default []
   */
   ...
   * @description key 是源字段，value 是新行目标字段。
   * @default {}
   */
   ...
   * @description 用于批量写入固定值，例如 {"status":"approved"}。
   * @default {}
   */
   ...
   * @description field/value 成对出现时，只更新一个字段。
   */
  field?: string

  /**
   * patch/set-field 使用的单字段值。
   * @description 与 field 配合使用。
   */
  value?: unknown

  /**
   * 当前内置动作成功后的链式动作。
   * @description 例如 append-row 成功后再 patch-current 清空草稿表单。
   */
  ...
  /**
   * 操作前确认类型。
   * @description 由 UI 消息组件解释，常见值为 warning、info、success、error。
   * @default "warning"
   */
  confirmType?: string
  ...
  /**
   * 当前行字段值匹配此对象时禁用按钮。
   * @description 常用于 status 等字段控制操作可用性。
   * @default {}
   */
  ...
  /**
   * 样式模板名。
   * @description 常见值如 primary、toolbar-danger、icon-add。
   */
  template?: string
  ...
  /**
   * 按钮类型。
   * @description 传给底层按钮组件的视觉类型。
   */
  buttonType?: string

  /**
   * 按钮尺寸。
   * @description Element Plus 按钮尺寸只允许 default、small、large。
   * @default "default"
   * @enumValue default 默认尺寸。
   * @enumValue small 小尺寸。
   * @enumValue large 大尺寸。
   */
  buttonSize?: string
