import type { ICapabilityContext } from './capability.js'
...
export function bindCapabilityContextOwner(owner: object, context: ICapabilityContext): void {
...
export function resolveCapabilityContextOwner(owner: object): ICapabilityContext | null {
	return OWNER_CONTEXTS.get(owner) ?? null
}

/** pageRoot 必须是页面根 DOM 元素（HTMLElement）。 */
export function bindPageRootCapabilityContext(pageRoot: HTMLElement, context: ICapabilityContext): void {
	PAGE_ROOT_CONTEXTS.set(pageRoot, context)
}

export function unbindPageRootCapabilityContext(pageRoot: HTMLElement): void {
	PAGE_ROOT_CONTEXTS.delete(pageRoot)
}

export function resolvePageRootCapabilityContext(pageRoot: unknown): ICapabilityContext | null {
