...
export class DataSet {
     ...
     */
    updateContextRows(context, table) {
        // 始终基于完整数据源
     ...
     * 更新数据行
     */
    updateRow(tableName, rowIndex, row) {
        const table = this.getTable(tableName);
     ...
     */
    cascadeUpdate(tableName, row, oldValues) {
        const table = this.getTable(tableName);
        if (!table)
            return [];
        // 查找需要级联更新的关系
        const relations = this.relations?.filter(rel => rel.parentTable === tableName && rel.cascadeUpdate) ?? [];
    ...
    cascadeDelete(tableName, row) {
        ...
        // 查找需要级联删除的关系
        const relations = this.relations?.filter(rel => rel.parentTable === tableName && rel.cascadeDelete) ?? [];
     ...
     */
    extractForeignKeyMap(expr) {
        const result = [];
        // 递归解析表达式
        const parse = (node) => {
            // 逻辑组合节点
            if ('children' in node && Array.isArray(node.children)) {
                node.children.forEach((child) => parse(child));
                return;
            }
            // 单一条件节点
     ...
     * 根据依赖类型获取父数据范围
     */
    getParentRows(parentContext, dependencyType) {
     ...
     * 过滤子表数据
     */
    filterChildRows(childRows, filterExpression, parentRows, _parentContext) {
        ...
        if ('op' in filterExpression && filterExpression.op === 'in' && parentRows.length > 0) {
            ...
            if (fieldName && 'field' in filterExpression) {
                ...
                const filtered = childRows.filter(childRow => parentValues.includes(childRow[childFieldName]));
                return filtered;
            }
        }
    ...
    applyRelation(relation) {
        ...
        const filteredRows = this.filterChildRows(sourceRows, relation.filterExpression, parentRows, parentContext);
        // 检查过滤结果是否变化
        ...
        const validSelectedRows = childContext.selectedRows?.filter(row => filteredRows.some(fr => isSameRow(fr, row))) ?? [];
     ...
     * 比较两个数据集是否相等（静态工具方法）
     */
    static areRowsEqual(rows1, rows2) {
    ...
    getTableDependencies(tableName) {
        ...
        const findParents = (currentTable) => {
            if (visited.has(currentTable))
                return;
            visited.add(currentTable);
            // 找到所有以 currentTable 为子表的关系
            const parentRelations = this.relations?.filter(rel => rel.childTable === currentTable) ?? [];
     ...
     * @returns 依赖条件是否满足
     */
    areDependenciesSatisfied(tableName) {
     ...
     * 从 JSON 加载
     */
    static fromJSON(json, dataLoader) {
     ...
     * @param callback 回调函数
     */
    subscribe(tableName, contextId = 'default', callback) {
     ...
     */
    notifySubscribers(tableName, contextId) {
        const table = this.getTable(tableName);
        if (!table)
            return;
     ...
     */
    getContext(tableName, contextId = 'default') {
        const table = this.getTable(tableName);
        if (!table)
            return undefined;
        // 默认上下文：DataTable 本身
     ...
     * 内部异步请求方法
     */
    async _requestTableDataAsync(tableName) {
        const table = this.getTable(tableName);
        // 检查是否为依赖表
        ...
        if (isDependentTable && table?.rows && table.rows.length > 0) {
            ...
            if (this.areDependenciesSatisfied(tableName)) {
                // 查找所有关联的 autoLoad 关系
                const relations = this.relations?.filter(rel => rel.childTable === tableName && rel.autoLoad) ?? [];
                if (relations.length > 0) {
    ...
    async loadTableData(tableName) {
        ...
        try {
            ...
            if (table) {
                ...
                const parentRelations = this.relations?.filter(rel => rel.childTable === tableName) ?? [];
     ...
     * 清理表的所有上下文的无效选中状态
     */
    cleanupInvalidSelections(table) {
     ...
     * 通知依赖已更新（触发事件，不自动加载）
     */
    notifyDependencyUpdated(tableName) {
     ...
     * 通知子表：父表数据已更新
     */
    notifyChildTables(parentTableName) {
        if (!this.relations)
            return;
        // 找到所有以此表为父表的子表
        const childRelations = this.relations.filter(rel => rel.parentTable === parentTableName);
     ...
     */
    recursiveClearChildTables(parentTableName, parentContextId = 'default') {
        if (!this.relations)
            return;
        // 找到所有以此表/上下文为父的子表
        const childRelations = this.relations.filter(rel => rel.parentTable === parentTableName &&
     ...
     * 应用与指定表相关的所有关系
     */
    applyRelationsForTable(tableName) {
        if (!this.relations)
            return;
        // 找到所有以此表为子表的关系
        const relations = this.relations.filter(rel => rel.childTable === tableName);
     ...
     * 更新相关联的子表
     */
    updateRelatedTables(parentTableName, parentContextId = 'default') {
        if (!this.relations)
            return;
        // 找到所有以此表为父表，且 parentContext 匹配的关系
        const relations = this.relations.filter(rel => {
            if (rel.parentTable !== parentTableName)
                return false;
            // 匹配 contextId
