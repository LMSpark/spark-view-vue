...
export class BindingContext {
    constructor(hostTable, contextId = 'default', dataSet) {
     ...
     * 设置当前选中行
     */
    setCurrentRow(row, skipNotify = false) {
        // 防重复检查 - 只比较引用
     ...
     */
    setSelectedRows(rows, skipNotify = false) {
        // 防重复检查：只比较引用
        const existingRows = this.selectedRows || [];
        const isSameSelection = (existingRows.length === rows.length &&
     ...
     * 应用排序表达式
     */
    applySorting(rows, sortExpression) {
        // 创建副本以避免修改原数组
        const sorted = [...rows];
        // 判断是单字段还是多字段排序
     ...
     * 单字段排序
     */
    sortByField(rows, field, direction) {
    ...
    cleanupInvalidSelections() {
        ...
        // 检查 currentRow
        if (this.currentRow) {
            const currentRowExists = contextRows.some(row => JSON.stringify(row) === JSON.stringify(this.currentRow));
            if (!currentRowExists) {
     ...
     * 从普通对象创建实例
     */
    static fromJSON(data, hostTable, contextId, dataSet) {
