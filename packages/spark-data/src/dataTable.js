 ...
 */
export class DataTable extends BindingContext {
    constructor(tableName, columns = [], dataSet) {
    ...
    getOrCreateContext(contextId) {
        ...
        if (!this.contexts[contextId]) {
            this.contexts[contextId] = new BindingContext(this.tableName, contextId, this.dataSet);
     ...
     * 从普通对象创建实例
     */
    static fromPlainObject(data, dataSet) {
        ...
        if (data.contexts) {
            if (Array.isArray(data.contexts)) {
                ...
                data.contexts.forEach((ctx, index) => {
                    ...
                    table.contexts[contextId] = BindingContext.fromJSON(ctx, table.tableName, contextId, dataSet);
                });
            }
            else {
                // 新格式：Record
                Object.entries(data.contexts).forEach(([contextId, ctxData]) => {
                    table.contexts[contextId] = BindingContext.fromJSON(ctxData, table.tableName, contextId, dataSet);
                });
            }
        }
        return table;
