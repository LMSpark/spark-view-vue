 ...
 * ```
 */
export function usePageDataSet(options) {
    const { pageData, context, originalRules, formApi, enableDataSet = true, dataLoader } = options;
    ...
    const initDataSet = () => {
        ...
        if (pageData.dataset && typeof pageData.dataset === 'object' && 'tables' in pageData.dataset) {
            ...
            const defaultDataLoader = dataLoader || (async (tableName) => {
                pageLogger.warn('使用默认 dataLoader，页面脚本应该注册自定义 dataLoader', { tableName });
                return [];
            });
            // 创建 DataSet (使用工厂模式)
            dataSet.value = DataSetManager.create(pageData.dataset, defaultDataLoader);
    ...
    const autoSubscribeTables = () => {
        if (!dataSet.value || !originalRules?.value)
            return;
        // 收集所有 (tableName, contextId) 组合
        const contexts = new Set();
        const extractContexts = (rules) => {
