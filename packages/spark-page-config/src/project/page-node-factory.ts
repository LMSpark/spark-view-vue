...
import type { BasePageContentLoader, PageContentLoaderOptions } from '../page-model/read/page-content-types'
...
import { ProjectConfigPageNodeModel, type PageNodeLike } from './project-node-model'
...
export class PageNodeFactory implements PageNodeFactoryLike {
  ...
  create(pageId: string): ProjectConfigPageNodeModel {
    const normalized = pageId.trim()
    if (!normalized) {
      throw new Error('pageId 不能为空')
    }
    const page = new ProjectConfigPageNodeModel({
      node: {
        id: normalized,
        title: normalized,
        nodeKind: 'page',
...
export function createPageNode(pageId: string, options: PageNodeFactoryOptions = {}): ProjectConfigPageNodeModel {
