...
import type { JsonObject, JsonValue } from '../capabilities/json-document'

// ── SECTION 1: 规则编辑器组件元数据 ──

export namespace RuleEditorComponent {
  ...
  export type MetadataSource = Metadata | Catalog
...
export function createRuleEditorComponentMetadata(
  source?: RuleEditorComponent.MetadataSource,
): RuleEditorComponent.Metadata {
  if (source === undefined) return RuleEditorComponent.EMPTY_METADATA
...
function isRuleEditorComponentMetadata(source: RuleEditorComponent.MetadataSource): source is RuleEditorComponent.Metadata {
  return 'types' in source && 'propNames' in source && 'propEnums' in source && 'typeLabels' in source && 'requiredProps' in source
}

function isConfigurableProp(prop: RuleEditorComponent.CatalogProp): boolean {
...
function inferDefaultFromProp(prop: RuleEditorComponent.CatalogProp): JsonValue {
...
function createTypeLabel(entry: RuleEditorComponent.CatalogEntry): string {
...
import type { AutoPopulateEntry, JsonPath, JsonTreePolicy } from '../capabilities/json-document'
import { ensureUniqueObjectKey, isRecord } from '../capabilities/json-document'
...
function getTypeLabelOptions(metadata: RuleEditorComponent.Metadata): Array<{ label: string; value: string }> {
...
function suggestChildKey(target: JsonObject, parentPath: JsonPath, metadata: RuleEditorComponent.Metadata): string {
...
export function createRuleTreePolicy(
  metadataSource?: RuleEditorComponent.MetadataSource,
): JsonTreePolicy {
