 ...
 *
 * ## 在 PageDesign 流程中的位置
 * node-tree 是 PageNode 结构工位，对应 100 步流程的步骤 89-92（页面结构）。
 * 它消费已确认的数据事实、组件 payload 指南和阶段检测结果，必要时也会把缺口
 * 反馈给 LLM 回到 dataset / payload-catalog 补齐。
 ...
 * 4. validateDataFirst            — 目录组件写入前，pagedata.json 需要至少已有业务 DataTable
 ...
 * - **数据事实门禁**：写目录组件前 pagedata.json 需要至少已有业务表，避免 UI 凭空装配
