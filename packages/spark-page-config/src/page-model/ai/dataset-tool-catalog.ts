 ...
 *
 * ## 在 PageDesign 流程中的位置
 * dataset 是 PageNode 数据事实工位：负责沉淀表、列、视图、行、关系、聚合等
 * 可确定的业务结构，为后续 UI 绑定和阶段检测提供事实来源。
 * 对应 100 步流程的步骤 21-88（数据规划与数据利用）。
 * 业务流程通常先形成主数据表；但 DataView 依赖和输入关系可在 UI/表单设计阶段
 * 通过 dataset 工具补齐，再由 node-tree/text-model 消费。
 *
 * ## 动作分组（~40 个）
 * ```
...
const DATASET_DIRECT_METHOD_NAMES: ReadonlySet<string> = new Set<DataSetCrudToolMethodName>([
  'undo',
  'redo',
  'clearHistory',
  'listTables',
...
export class PageDesignDatasetAiModule extends AiModule {
  ...
  protected override async runFunction(
    ...
    return this.serviceResultToOperationResult(
      await this.service.runDatasetAction(
...
function resolveDatasetMethodName(actionName: string): DataSetCrudToolMethodName {
  if (isDatasetActionMethodAlias(actionName)) return DATASET_ACTION_METHOD_NAMES[actionName]
  if (isDatasetDirectMethodName(actionName)) return actionName
  throw new Error(`DataSetCrudTool action is not mapped: ${actionName}`)
}
...
function isDatasetDirectMethodName(actionName: string): actionName is DataSetCrudToolMethodName {
