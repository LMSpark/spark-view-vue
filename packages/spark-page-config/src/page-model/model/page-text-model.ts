...
export class PageTextModel {
  ...
  constructor(
    private readonly _fileName: PageNodeFileName,
    initialText = '',
  ) {
  ...
  async load(
    ...
    const result = await contentLoader.loadPageFileContent(pageId, this._fileName, {
  ...
  async restoreVersion(command: PageFileRestoreCommand): Promise<void> {
    ...
    const result = await contentLoader.loadPageFileContent(pageId, this._fileName, { forceReload: true })
    if (!result.success) {
