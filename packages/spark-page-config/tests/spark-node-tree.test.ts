...
import { SPARK_PAGE_NODE_TYPE, SPARK_PAGE_ROOT_ID, SparkNodeTree } from '@spark-view/spark-page-config/page/spark-node-tree'
import type { SparkNode } from '@spark-view/spark-page-config/page/spark-node'
...
describe('SparkNodeTree', () => {
  it('应该围绕单个 root 实例提供查询 API', () => {
    ...
    expect([...tree.collectDataViewKeys()]).toEqual(['Users@default'])
  ...
  it('setProps 应支持 merge 与 replace', () => {
    ...
    const replaced = tree.setProps({
      componentId: 'table',
      props: { dataViewKey: 'Orders@default' },
      merge: false,
    })

    expect(replaced.node.props).toEqual({ dataViewKey: 'Orders@default' })
  })
