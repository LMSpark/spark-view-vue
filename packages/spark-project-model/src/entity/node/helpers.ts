...
import { normalizeNavRoot, normalizePageIdFromPath } from '../../service/navigation/editing'
import type { NodeKind, ProjectRequirementConstraint } from '../../contract/node'
...
export function readProjectPlanningNodeKind(node: NavNode | null | undefined): NodeKind | null {
...
export function readAllowedProjectPlanningChildKinds(p: 'project' | NodeKind): readonly NodeKind[] {
...
export function readProjectNodeRequirement(node: NavNode | null | undefined): string { return node?.description?.trim() ?? '' }

export function createProjectRequirementConstraint(node: NavNode | null | undefined): ProjectRequirementConstraint | null {
  const d = readProjectNodeRequirement(node)
  if (!node || !d) return null
  return { nodeId: node.id, title: node.title, nodeKind: node.nodeKind ?? 'page', description: d }
}

export function appendProjectRequirementConstraint(c: readonly ProjectRequirementConstraint[], node: NavNode | null | undefined): ProjectRequirementConstraint[] {
  const n = createProjectRequirementConstraint(node)
  return n === null ? [...c] : [...c, n]
}

export function formatProjectRequirementConstraints(c: readonly ProjectRequirementConstraint[]): string {
