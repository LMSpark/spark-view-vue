...
import { CATALOG_OVERRIDES, CATALOG_ADDENDUMS, COMPONENT_CATEGORIES } from './component-props-supplement'
...
`
  ...
  /* ---------------------------------------------------------------------------
   * Props 目录生成（AST 提取 + 补充数据合并）
   *
   * 扫描 renderer 字段组件 + features 组件，提取 Props JSDoc，
   * 与 component-props-supplement.ts 合并后生成 component-props-catalog.ts。
   * 容器组件由补充数据全量覆盖（rule.json 格式与 Props 名不同）。
   * 字段组件由 AST 自动生成，补充数据追加透传 Props 说明。
   * ------------------------------------------------------------------------- */

  /**
   * 额外扫描 Renderer 组件（仅用于目录生成，不参与注册）
   */
  private scanRendererComponents(root: string): Array<{
    absolutePath: string
    relativePath: string
    skillType: string
  ...
  /**
   * 生成 component-props-catalog.ts 并写入 spark-ai 包
   */
  generatePropsCatalog(): void {
    if (!this.viteConfig) return

    const root = this.viteConfig.root
    const renderers = this.scanRendererComponents(root)
    ...
    for (const comp of renderers) {
      ...
      try {
        ...
        astEntries[comp.skillType] = this.formatAstPropsEntry(
          comp.skillType,
    ...
    // features 组件也扫描（已在 this.components 中）
    for (const comp of this.components) {
      if (comp.name in CATALOG_OVERRIDES) continue
      if (comp.name in astEntries) continue // 已有 renderer 条目

      try {
        const source = readFileSync(comp.absolutePath, 'utf-8')
        const api = extractComponentApi(source, comp.path, comp.name)
        if (!api) continue

        const desc = comp.skillMeta?.description ?? buildImplicitSkillDescription(comp.absolutePath, comp.name)
        astEntries[comp.name] = this.formatAstPropsEntry(comp.name, desc, api)
      } catch {
        // 跳过
      }
    }

    // 合并：overrides > AST + addendums
    ...
    // 添加纯 addendum 条目（无 AST 也无 override 的情况，理论上不存在但防御性处理）
    ...
    for (const key of sortedKeys) {
      ...
      if (cat === 'meta') continue // 元概念不进入注册表
      if (cat === 'container') containers.push(key)
      else if (cat === 'group') groups.push(key)
      else if (key.startsWith('r-')) fields.push(key) // AST 自动提取的默认为 field
    }
...
`
    ...
    const outputPath = resolve(root, 'packages/spark-ai/src/component-props-catalog.ts')
    try {
      writeFileSync(outputPath, output, 'utf-8')
      logger.info(`📋 Props 目录已生成: ${relative(root, outputPath)} (${sortedKeys.length} 条目)`)
    } catch (e) {
...
export function sparkComponentsPlugin(
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      // 生成 Props 目录（AST + 补充数据合并）
      analyzer.generatePropsCatalog()
    },
