<template>
  <div>
    ...
    <el-form-item label="隐藏" class="switch-item">
      <el-switch v-model="state.navDraft.hidden" :disabled="isSubPage" @change="state.markNavDirty" />
    ...
    <el-form-item label="禁用" class="switch-item">
      <el-switch v-model="state.navDraft.disabled" @change="state.markNavDirty" />
      <span class="switch-item__hint">保留显示但不可点击（仅 true 持久化，false 为默认值不落库）</span>
    </el-form-item>
    <el-form-item label="后置分割线" class="switch-item">
      <el-switch v-model="state.navDraft.dividerAfter" @change="state.markNavDirty" />
      <span class="switch-item__hint">在当前节点后显示分割线</span>
    </el-form-item>

    <el-divider content-position="left">权限控制</el-divider>
    <el-form-item label="权限模式">
      <el-radio-group v-model="state.navDraft.permissionMode" @change="state.markNavDirty">
...
const isSubPage = computed(() => props.state.navDraft.nodeKind === 'sub-page')
</script>
